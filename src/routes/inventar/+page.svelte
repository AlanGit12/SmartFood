<script>
	...
	$: filteredProducts = products.filter((product) => {
		...
		return (
			...
			matchesExpiry(product)
		);
	});

	// Kennzahlen (auf Basis der gefilterten Produkte)
	$: totalProducts = filteredProducts.length;

	$: expiringSoon = filteredProducts.reduce(
		(sum, product) =>
			sum +
			product.variants
...
<section class="inventar-header">
	...
	<div class="toolbar">
		...
		<select
			...
			aria-label="Nach Aufbewahrungsort filtern"
		>
...
<section class="kpi-row">
	...
	<SummaryCard
		...
		value={expiringSoon}
		subtitle="Stück mit kurzem Ablaufdatum"
		icon="⚠️"
		variant="warning"
	/>
...
{#if groupByLocation}
	{#if groupedProducts.length === 0}
		<p class="empty">
			Keine Produkte gefunden. Passe Suche oder Filter an.
		</p>
	{:else}
